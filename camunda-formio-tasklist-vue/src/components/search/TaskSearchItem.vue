...
export default class TaskSearchItem extends Vue {
    ...
    deleteSearchQueryElement (query: any, index: number) {
      this.$root.$emit('call-deleteSearchQueryElement', {query: query, index: index})
    }
    updateSearchQueryElement (updateSearch: any, index: number) {
      this.$root.$emit('call-updateSearchQueryElement', {updateSearch: updateSearch, index: index});
    }
    ...
    updateSearchQueryOperators (operator: string, index: number) {
      this.$root.$emit('call-updateSearchQueryOperators', {operator: operator, index: index})
    }

    setSearchQueryValue (item: any, query: any, operator: string, idx: number) {
      this.$root.$emit('call-setSearchQueryValue', {item: item, query: query, operator: operator, idx: idx})
    }
